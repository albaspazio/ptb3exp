...
function testvideo()
    ...
    try
        ...
        for v=1:nvideo
            ...
            file_video              = fullfile(input_video_folder, [video_list{v} video_file_extension]);
            file_audio              = fullfile(input_audio_folder, [audio_list{v} audio_file_extension]);
            trigger_value           = triggers_list{v};

            sound_obj.audio_file    = file_audio;
            sound_obj.audio_frame   = 45; ...floor(video_matrix.frames/2);
            sound_obj.ao            = audio_obj;

            showMovie(file_video, window1, 'sound_obj', sound_obj);
